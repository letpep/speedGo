...
local pwd =  ngx.var.arg_pwd

local res, err = httpc:request_uri('http://10.102.4.252:3000/login', {
    method = "POST",
    headers = {
        ["Content-Type"] = "application/json",
    },
    body = '{"user":"'..user..'","email":"","password":"'..pwd..'"}'
})
...
if res.body == '{"message":"Logged in"}' then
    ngx.say('ok')
end
